...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'sem_A', 'rec_sem_A', 'D_B', 'G_B', 'cycle_B', 'idt_B', 'sem_B', 'rec_sem_B', 'hue']
    ...
    def compute_hue_loss(self, real_B, fake_B):
        ...
        fake_b_boxes = self.box_down(fake_B)
        cos_sim = self.cos_sim(real_b_boxes, fake_b_boxes)
        hue_loss=1-cos_sim
        return hue_loss
    ...
    def box_down(self, img):
        ...
        #h=hsv[:,:,0]
        #h1 = np.expand_dims(h, axis=0)
        #h2=torch.from_numpy(h1)
        #h2=h2.float()
        #m = torch.nn.AdaptiveAvgPool2d(9)
        #output = m(h2)
        h1 = h.unsqueeze_(0)
        m = torch.nn.AdaptiveAvgPool2d(9)
        output = m(h1)
        hue_list=output.squeeze()
        #print(hue_list)
        #print(hue_list.shape)
        #hue_list = output.numpy()
        
        hue_list_new=hue_list.cpu().detach().numpy()
        #hue_list_new=hue_list_new.tolist()
        #print(hue_list_new)
        hue_list_new=hue_list_new.ravel()
        #print(hue_list_new)
        hue_tensor=torch.Tensor(hue_list_new) 
        hue_tensor=hue_tensor.unsqueeze(0)
        #print(hue_tensor)
        #print(hue_tensor.shape)
        return hue_tensor
    

    def cos_sim(self, hue1, hue2):
        cos_sim=torch.cosine_similarity(hue1,hue2).item()
        #print(cos_sim)
        return cos_sim
